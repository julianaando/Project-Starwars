...
describe('Testa a aplicação', () => {
  beforeEach(async () => { // antes de cada teste, faça:
    jest.spyOn(global, 'fetch').mockResolvedValue({ // substitui o fetch do global
      json: jest.fn().mockResolvedValue(mockPlanets), // substitui o json do fetch
    });

    await act(async () => { // aguarda o fetch ser resolvido
  ...
  afterEach(() => { // depois de cada teste, faça:
    jest.restoreAllMocks(); // restaura o fetch do global
  });

  it('Testa os inputs da tela', () => {
  ...
  it('Testa se os 3 filtros de dropdown e valor estão funcionando', () => {
  ...
  it('Testa o filtro de Operadores `maior que`', () => {
    ...
    fireEvent.change(screen.getByTestId('value-filter'), { target: { value: '10' } });
    fireEvent.click(screen.getByTestId('button-filter'));
    expect(screen.getByText('Kamino')).toBeInTheDocument();
  });
  it('Testa o filtro de Operadores `igual a`', () => {
